...
## Algorithm to sample from both Zeta and Phi          ##
#########################################################

test_phi.8MH3 = numeric(numIter)
test_zeta     = numeric(numIter)
...
for (ii in 1:numIter){
  ...
  # Update new parameters using the MH Algorithm 
  phi.8MH3[1:K] = mh_algo(dataMat, 
                          phi.8MH3[1:K])
  
  dataMat$theta = phi.8MH3[dataMat$cluster]
  
  # Storing params value for observation 
  test_phi.8MH3[ii] = phi.8MH3[dataMat$cluster[test_obs]]
  
  # Update the new zeta using the MH Algorithm
  zetaUpdate = mh_algoZeta(dataMat)
  
  dataMat$zeta = zetaUpdate
  
  test_zeta[ii] = zetaUpdate
  
  dataMat$pi = dataMat$theta*dataMat$zeta
