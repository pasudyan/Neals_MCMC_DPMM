...
for (ii in 1:numIter){
  ...
  if(ii > burnIn){
    ...
    phi.8MH[(K+1):(K+m)] <- rnorm(m, mu_base, sd_base)
    
    #Calculating the predictive prob
    probPoints <- unlist(lapply(1:length(points), function(k){
      (sum(exp(
        log(counts[1:K]) + 
          dnorm(points[k],phi.8MH[1:K],sd_obs,log=TRUE))) + 
         sum(exp(
           log(alpha/m) + 
             dnorm(points[k],phi.8MH[(K+1):(K+m)],sd_obs,log=TRUE) +
             dnorm(phi.8MH[(K+1):(K+m)], sd_base, log=TRUE)))
      )/(length(mixDat)+alpha)
    }))
    
    val_alg8MH[(ii-burnIn),] <- probPoints
  }
...
quants = apply(val_alg8MH, 2, quantile, probs=c(0.95, 0.05))
meanCol = colMeans(val_alg8MH)

datPoints8MH <- data.frame(points = points, 
                         lower=quants[1,],
                         upper=quants[2,], 
                         mean=meanCol)
