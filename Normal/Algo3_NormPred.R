...
for (ii in 1:numIter){
  ...
  test_phi.3[ii] = phi.3[cluster[test_obs]]
    ...
    if (ii > burnIn){
      ...
      #sampling the parameter for a new cluster
      phi.3[K+1] = rnorm(1, mu_base, sd_base)
      
      #Calculating predictive probabilities
      probPoints = unlist(lapply(1:length(points), function(k){
        (sum(exp(
          log(counts[1:K]) + 
            dnorm(points[k],phi.3[1:K],sd_obs,log=TRUE))) + 
          exp(log(alpha) + 
            dnorm(points[k],phi.3[K+1],sd_obs,log=TRUE) +
            dnorm(phi.3[K+1], sd_base, log=TRUE))
        )/(length(mixDat)+alpha)
      }))
      
      val_alg3[(ii-burnIn),] = probPoints
    }
...
quants = apply(val_alg3, 2, quantile, probs=c(0.95, 0.05))
meanCol = colMeans(val_alg3)

datPoints3 <- data.frame(points = points, 
                         lower=quants[1,],
                         upper=quants[2,], 
                         mean=meanCol)
